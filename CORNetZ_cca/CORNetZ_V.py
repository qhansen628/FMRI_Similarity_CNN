...
class CORNetZV(object):
    ...
    def __init__(self, x, keep_prob, num_classes, skip_layer, v_batch_size=50): 
        ...
        with tf.compat.v1.variable_scope("weights", reuse=tf.compat.v1.AUTO_REUSE) as scope:
            
            self.W1 = tf.compat.v1.get_variable("W1", shape=[7, 7, 3, 64])
            self.b1 = tf.compat.v1.get_variable("b1", shape=[64])
            self.W2 = tf.compat.v1.get_variable("W2", shape=[3, 3, 64, 128])
            self.b2 = tf.compat.v1.get_variable("b2", shape=[128])
            self.W3 = tf.compat.v1.get_variable("W3", shape=[3, 3, 128, 256])
            self.b3 = tf.compat.v1.get_variable("b3", shape=[256])
            self.W4 = tf.compat.v1.get_variable("W4", shape=[3, 3, 256, 512])
            self.b4 = tf.compat.v1.get_variable("b4", shape=[512])
            self.W5 = tf.compat.v1.get_variable("W5", shape=[8192, 4096])
            self.b5 = tf.compat.v1.get_variable("b5", shape=[4096])
            self.W6 = tf.compat.v1.get_variable("W6", shape=[4096, 4096])
            self.b6 = tf.compat.v1.get_variable("b6", shape=[4096])
            self.W7 = tf.compat.v1.get_variable("W7", shape=[4096, self.NUM_CLASSES])
            self.b7 = tf.compat.v1.get_variable("b7", shape=[self.NUM_CLASSES])
    ...
    def forward(self):
        ...
        pool1 = tf.nn.max_pool2d(input=norm1, ksize=[1, 7, 7, 1], strides=[1, 2, 2, 1], padding='VALID', name='pool1')
        ...
        pool2 = tf.nn.max_pool2d(input=norm2, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID', name='pool2')
        ...
        pool3 = tf.nn.max_pool2d(input=norm3, ksize=[1, 3, 3, 1], strides = [1, 2, 2, 1], padding='VALID', name='pool3')
        ...
        pool4 = tf.nn.max_pool2d(input=norm4, ksize=[1, 3, 3, 1], strides = [1, 2, 2, 1], padding='VALID', name='pool4')
        ##decoder
        flattened = tf.reshape(pool4, [128, 8192])

        with tf.compat.v1.variable_scope("weights", reuse=tf.compat.v1.AUTO_REUSE) as scope:
            fc6 = fc(flattened, self.W5, self.b5, name='fc6')
        dropout6 = tf.nn.dropout(fc6, 1 - (self.KEEP_PROB), seed=0)
        dropout6 = dropout6
        with tf.compat.v1.variable_scope("weights", reuse=tf.compat.v1.AUTO_REUSE) as scope:
            fc7 = fc(dropout6, self.W6, self.b6, name='fc7')
        dropout7 = tf.nn.dropout(fc7, 1 - (self.KEEP_PROB), seed=0)
...
def conv(x, W, b, strides, padding, name):
    conv = tf.nn.conv2d(input=x, filters=W, strides=strides, padding=padding, name=name)
...
def fc(x,W, b, name, relu=True):
    ...
    act = tf.compat.v1.nn.xw_plus_b(x, W, b, name=name)
