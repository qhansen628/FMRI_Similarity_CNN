...
def compute_cost_V(act1, act2, Y):
    ...
    for i in range(act1.shape[0]):
        with tf.compat.v1.Session() as sess: 
            rsa = tf.compat.v1.losses.cosine_distance(tf.nn.l2_normalize(act1[i], 0), tf.nn.l2_normalize(act2[i],0), axis=0)
            #c = tf.reduce_sum(tf.math.abs(tf.math.subtract(rsa,Y[i])))
            c = tf.reduce_sum(tf.math.squared_difference(rsa,Y[i]))
            cost += c 
    return cost
...
def get_args(which, v_ratio):
    ...
    args['v4_file'] = 'ROI_train_distance.txt'
    args['it_file'] = 'ROI_train_distance.txt'
...
def run_net(args, test): 
    ...
    if args['v'] != 'None':  
        ...
        img2 = tf.compat.v1.placeholder(tf.float32, [args['v_batch_size'], 227, 227, 3])
        RSA = tf.compat.v1.placeholder(tf.float32, [args['v_batch_size']])
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        for epoch in range(args['num_epochs']):
            ...
            for val_step in range(val_batches_per_epoch):
                ...
                test_fine_acc += acc
                if args['comet']:
                    #log validation metrics to comet.ml experiment
...
if __name__ == "__main__":
    ...
    parser.add_argument('--v1_file',help='text_file for v1 similarity data')
    parser.add_argument('--v4_file',help='text_file for v4 similarity data')
    parser.add_argument('--it_file',help='text_file for it similarity data')
    ...
    optional_hyperparams = {'v1_file':argp.v1_file, 'v4_file':argp.v4_file, 'it_file':argp.it_file,
                            ...
                            'learning_rate':argp.l_rate,'dropout_rate':argp.dropout}
