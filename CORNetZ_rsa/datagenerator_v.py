...
class ImageDataGeneratorV(object):
    ...
    def _parse_function_train(self, filename1, filename2, RSA):
        ...
        img1_decoded = tf.image.decode_jpeg(img1_string, channels=3)
        img1_resized = tf.image.resize(img1_decoded, [int(self.img_size), int(self.img_size)])

        img2_string = tf.io.read_file(filename2)
        img2_decoded = tf.image.decode_jpeg(img2_string, channels=3)
        img2_resized = tf.image.resize(img2_decoded, [int(self.img_size), int(self.img_size)])
        """
